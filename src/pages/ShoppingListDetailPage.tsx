...
const ShoppingListDetailPage: React.FC = () => {
  ...
  const handleProductSelect = (product: ProductSearchResult) => {
    console.log("🛒 ShoppingListDetailPage: Product selected, adding automatically:", product.name);
    addItem(product.name, 1, undefined, undefined, product.id, product.slug);
  };

  const handleCustomItemSelect = (itemName: string) => {
    console.log("🛒 ShoppingListDetailPage: Custom item selected, adding automatically:", itemName);
    addItem(itemName, 1);
  };
  ...
  const handleIncrementQuantity = (itemId: string) => {
    console.log("🛒 ShoppingListDetailPage: Incrementing quantity for item:", itemId);
  ...
  const handleDecrementQuantity = (itemId: string) => {
    console.log("🛒 ShoppingListDetailPage: Decrementing quantity for item:", itemId);
    const item = listData?.items.find(i => i.id === itemId);
    if (item) {
      const currentQuantity = item.quantity || 1;
