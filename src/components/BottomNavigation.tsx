...
const BottomNavigation: React.FC<BottomNavigationProps> = ({
  ...
  const handleFavoritesClickFromDrawer = useCallback(() => {
    handleDrawerClose();
    setTimeout(() => {
      handleFavoritesClick();
    }, 150);
  }, [handleDrawerClose, handleFavoritesClick]);
  ...
  const navItemClasses = useMemo(() => ({
    ...
    lists: `mb-1 ${currentView === 'shopping-lists' ? 'text-blue-500' : 'text-gray-400'}`,
  ...
  const textClasses = useMemo(() => ({
    ...
    lists: `text-xs ${currentView === 'shopping-lists' ? 'text-blue-500 font-medium' : 'text-gray-500'}`,
    ...
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t z-50" style={{ height: 'var(--bottom-nav-height)' }}>
        <div className="flex justify-around items-center h-full px-1 py-2">
          ...
          <div
            className="flex flex-col items-center py-2 cursor-pointer"
            onClick={handleShoppingListsClick}
            data-testid="lists-nav"
          >
            <FileText
              size={24}
              className={navItemClasses.lists}
            />
            <span className={textClasses.lists}>
              {t('navigation.lists')}
            </span>
          </div>
          <div
            ...
            {isAuthenticated ? (
              <>
                ...
                <Button
                  ...
                  onClick={handleFavoritesClickFromDrawer}
                  disabled={isLoggingOut}
                >
